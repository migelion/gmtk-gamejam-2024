...
func launch_scarab_drop(object: Node2D, side: Side) -> void:
	var target: Node2D
...
func launch_random_burden() -> void:
	var choice: float = rng.randf_range(0, totalSpawnRate)
	var toLaunch: Node = burdens[burdens.size() - 1]
	for b in burdens:
		choice -= b.spawnRate
		if choice < 0:
			toLaunch = b
	...
	var duplicated: Node = toLaunch.duplicate()
...
func launch_random_scavenger() -> void:
	var liveBurdens: Array = scene.get_children().filter(is_burden)
	var idx: int = rng.randi_range(0, liveBurdens.size() - 1)
	...
	match side:
		...
		Side.RIGHT:
			despawnTarget = %ScarabDespawnRight
			spawn = %ScarabSpawnRight
	var scarab: Node2D = create_scarab(target, despawnTarget)
	scarab.state = Scarab.State.SCAVENGING
	launch_scarab(scarab, spawn.global_position)

func is_burden(obj) -> bool:
